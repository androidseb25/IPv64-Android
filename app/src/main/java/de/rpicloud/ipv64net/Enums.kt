...
enum class StatusType(val type: StatusTypeClass) {
    ...
    StabilizePhase(StatusTypeClass(5, "Stabilisierungsphase", R.color.transparent, R.color.ipv64_red)),
    Fallback(StatusTypeClass(0, "Aktualisiert", R.color.transparent, R.color.ipv64_gray)),
    Transparent(StatusTypeClass(-1, "", R.color.transparent, R.color.transparent))
}

enum class Unit(val Unit: MTimeUnit) {
    Minuten(MTimeUnit(1, "Minute/n")),
    Stunden(MTimeUnit(2, "Stunde/n")),
    Tage(MTimeUnit(3, "Tag/e"))
}
